...
export const sfx = {
  ...
  /** ส่งการบ้าน */
  // เสียงที่เคยขาด — เหตุการณ์สำคัญที่เกิดขึ้นแล้วเงียบสนิท
  /** ความเชื่อใจเพิ่ม — ต่างจาก gain ตรงที่มันช้ากว่าและลึกกว่า */
  trust:   () => { tone(330, 0.3, "sine", 0.07); setTimeout(() => tone(494, 0.45, "sine", 0.06), 130); },
  /** ความเชื่อใจหาย — เสียงที่ไม่มีปลาย */
  broke:   () => { tone(294, 0.5, "triangle", 0.07, 196); },
  /** ผิดนัดเพราะไปหาคนอื่น */
  stood:   () => { tone(392, 0.25, "sine", 0.05, 294); setTimeout(() => tone(247, 0.5, "sine", 0.05, 196), 180); },
  /** เรื่องที่เกิดขึ้นลับหลัง — เสียงที่บอกว่า "มีอะไรเกิดขึ้นตอนเราไม่อยู่" */
  news:    () => { tone(587, 0.18, "triangle", 0.06); setTimeout(() => tone(440, 0.4, "sine", 0.06, 370), 120); },
  /** ตัวละครหยิบเรื่องเก่ามาพูดเอง */
  recall:  () => { [494, 587].forEach((f, i) => setTimeout(() => tone(f, 0.5, "sine", 0.05), i * 150)); },
  /** ฝืนทำทั้งที่แรงไม่พอ */
  push:    () => { noise(0.2, 0.08, 300, 0.7); tone(196, 0.35, "sawtooth", 0.06, 150); },
  /** ประกาศอันดับหน้าห้อง */
  board:   () => { [392, 523, 440].forEach((f, i) => setTimeout(() => tone(f, 0.35, "triangle", 0.07), i * 110)); },
  homework: () => { tone(494, 0.16, "triangle", 0.07); setTimeout(() => tone(587, 0.2, "sine", 0.06), 70); },
  /** เดินเข้าไปในที่ใดที่หนึ่ง — เบามาก เพราะเกิดบ่อยกว่าเสียงอื่นทั้งหมด */
  step:    () => { tone(330, 0.07, "sine", 0.028); },
  /** เปิดประตูเข้าไปแล้วมีคนอยู่ตรงนั้น — เสียงของความบังเอิญ */
  meet:    () => { tone(587, 0.12, "sine", 0.05); setTimeout(() => tone(784, 0.16, "sine", 0.045), 70); },
  /** ได้รู้จักกับใครสักคนเป็นครั้งแรกทั้งเทอม — เกิดได้แค่หกครั้งในเกมหนึ่งรอบ */
  newface: () => { [523, 659, 880].forEach((f, i) => setTimeout(() => tone(f, 0.55, "sine", 0.075), i * 105)); },
};
